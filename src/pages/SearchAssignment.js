...
import {
  getEmergencyById,
  addFindingToEmergency,
  subscribeToFindings,
  getFindingsForEmergency,
...
const SearchAssignment = () => {
  ...
  const [findings, setFindings] = useState([]); // Add this state variable at the top with other state variables
  ...
  // Function to refresh findings manually
  const refreshFindings = async () => {
    try {
      if (emergency?.id) {
        console.log(
          "Manually refreshing findings for emergency:",
          emergency.id
        );
        const newFindings = await getFindingsForEmergency(emergency.id);
        console.log("Fetched findings:", newFindings.length);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (assignmentData) {
          ...
          console.log("Fetched emergency data:", emergencyData);
          setEmergency(emergencyData);

          // Also set findings separately
          if (emergencyData?.findings) {
            console.log(
              "Setting initial findings:",
              emergencyData.findings.length
            );
            setFindings(emergencyData.findings);
          }

          // Set up subscription to findings
          const unsubscribeFindings = subscribeToFindings(
            assignmentData.emergencyId,
            (newFindings) => {
              console.log(
                "Findings subscription update received:",
                newFindings.length,
                "findings"
              );
              // Update findings state directly
              setFindings(newFindings);
            }
          );
    ...
    const unsubscribe = subscribeToSearchAssignment(id, (data) => {
      console.log("Real-time update received for assignment:", data?.id); // Debug log
      if (data) {
        setAssignment(data);
      }
    });
    ...
    return () => {
      ...
      if (findingsSubscription) {
        console.log("Cleaning up findings subscription");
  ...
  const handleSubmitFinding = async (e) => {
    ...
    try {
      ...
      console.log("Submitting finding for emergency:", emergency.id);
      ...
      const findingId = await addFindingToEmergency(emergency.id, findingData);
      console.log("Finding added with ID:", findingId);

      // Manually add the finding to our local state while waiting for the subscription
      const newFinding = {
        id: findingId,
        ...findingData,
      };

      // Update both findings state and emergency state
      setFindings((prev) => [newFinding, ...prev]);
      setEmergency((prev) => {
        if (prev) {
          const updatedFindings = prev.findings
            ? [newFinding, ...prev.findings]
            : [newFinding];
      ...
      // Also refresh findings from server after a small delay
      setTimeout(() => {
        refreshFindings();
      }, 1000);
            ...
            <div>
              ...
              <div className="mt-4 mb-2 flex justify-end">
                <button
                  ...
                  className="bg-blue-100 text-blue-700 hover:bg-blue-200 text-sm px-3 py-1 rounded flex items-center"
                >
                  <svg
